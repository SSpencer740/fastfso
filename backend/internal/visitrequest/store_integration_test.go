...
func createVisitRequest(t *testing.T, store *visitrequest.Store, ctx context.Context, e env) *visitrequest.VisitRequest {
	...
	created, err := store.Create(ctx, visitrequest.CreateParams{
		...
		DestinationName:  "Lockheed Martin",
		DissSmoCode:      "LM001",
...
func TestStore_Create(t *testing.T) {
	...
	assert.Equal(t, "Lockheed Martin", vr.DestinationName)
...
func TestStore_Get(t *testing.T) {
	...
	assert.Equal(t, "Lockheed Martin", got.DestinationName)
...
func TestStore_List(t *testing.T) {
	...
	for i, dest := range []string{"Lockheed Martin", "Raytheon", "Boeing"} {
...
func TestStore_List_StatusFilter(t *testing.T) {
	...
	assert.Len(t, rows, 1)
	assert.Equal(t, "Raytheon", rows[0].DestinationName)
	...
	assert.Equal(t, 1, total)
	assert.Len(t, rows, 1)
	assert.Equal(t, "Lockheed Martin", rows[0].DestinationName)
}
...
func TestStore_List_UserScope(t *testing.T) {
	...
	_, err := store.Create(ctx, visitrequest.CreateParams{
		...
		DestinationName:  "Northrop Grumman",
		DissSmoCode:      "NG001",
